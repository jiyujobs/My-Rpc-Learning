...
/**
 * Netty 中处理RpcRequest的Handler
 *
 */
public class NettyServerHandler extends SimpleChannelInboundHandler<RpcRequset> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, RpcRequset msg) throws Exception {
        try {
            ...
            if(ctx.channel().isActive() && ctx.channel().isWritable()){
                ctx.writeAndFlush(RpcResponse.success(result, msg.getRequsetId()));
            } else {
                logger.error("通道不可写");
            }
        } finally {

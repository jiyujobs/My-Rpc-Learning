package top.qiaoxianwen.rpc.transport.netty.client;
...
import top.qiaoxianwen.rpc.factory.SingletonFactory;
...
public class NettyClient implements RpcClient{
    ...
    @Override
    public CompletableFuture<RpcResponse> sendRequest(RpcRequset rpcRequset) {
        if(serializer == null){
        ...
        try {
            ...
            unprocessedRequests.put(rpcRequset.getRequsetId(), resultFuture);
            channel.writeAndFlush(rpcRequset).addListener((ChannelFutureListener) future1 -> {
                if(future1.isSuccess()){
                    logger.info(String.format("客户端发送消息: %s", rpcRequset.toString()));
                } else {
            ...
            unprocessedRequests.remove(rpcRequset.getRequsetId());
